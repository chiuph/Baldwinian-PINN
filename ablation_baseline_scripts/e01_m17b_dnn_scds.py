## [Steady-State Convection Diffusion Problem] [DNN] Meta-Modelling
## Version 1 : 2023/8/11
...
def run(model_type=1, learning_rate=5e-3, seed=0, location='results_meta_pdes'):
    ...
    for v in range(1, 111): 
        ...
        #print('v = %03d | (SGD DNN) MSE = %.1e | SSR = %.1e  MSE = %.1e'%(round(v), mse, ssr_s, mse_s));
        _res = [round(v), mse]
        _ssr = [ssr_s]
        for lamb in lambs:
            # SGD PINN + Pseudo-inverse
            u_sp, ssr_sp, mse_sp = compute_ls_task(batch_data_x, batch_data_y, v, lamb)
            #print('          lambda = %.e | (SGD DNN + Pseudo-inverse) SSR = %.1e  MSE  = %.1e | (FrPINN) MSE = %.1e'%(lamb, ssr_sp, mse_sp, mse_rp));
            _res = _res + [mse_sp]
            _ssr = _ssr + [ssr_sp]
    ...
    his_test.to_csv(os.path.join(folder, 'scds_dnn_m%d_his_lr%.e_seed-%02d.csv'%(model_type, learning_rate, seed)))
    res_test.to_csv(os.path.join(folder, 'scds_dnn_m%d_res_lr%.e_seed-%02d.csv'%(model_type, learning_rate, seed)))    
