## [Projectile Motion] [PINN] Meta-Modelling
## Version 1 : 2023/8/12
...
def run(model_type=1, learning_rate=5e-3, seed=0, location='results_meta_pdes'):
    ...
    def eval_loss(params, inputs, labels):
        ...
        # IC
        _ic = jnp.where(jnp.equal(inputs[:,0], 0), 1, 0).reshape(-1, 1)
        ic_mse_1 = jnp.sum(jnp.square((u0 - x_t)*_ic)) / jnp.sum(_ic)
        ic_mse_2 = jnp.sum(jnp.square((v0 - y_t)*_ic)) / jnp.sum(_ic)
        ic_mse_3 = jnp.sum(jnp.square((x0 - x)*_ic)) / jnp.sum(_ic)
        ic_mse_4 = jnp.sum(jnp.square((y0 - y)*_ic)) / jnp.sum(_ic)
        ic_mse = ic_mse_1 + ic_mse_2 + ic_mse_3 + ic_mse_4
        # PDE 1: x_tt + R*x_t = 0
        # PDE 2: y_tt + R*y_t = -g
    ...
    his_test.to_csv(os.path.join(folder, 'pms_pinn_m%d_his_lr%.e_seed-%02d.csv'%(model_type, learning_rate, seed)))
    res_test.to_csv(os.path.join(folder, 'pms_pinn_m%d_res_lr%.e_seed-%02d.csv'%(model_type, learning_rate, seed)))    
